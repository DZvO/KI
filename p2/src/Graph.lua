...
--           pat - the delimited
--    @return a table of two vertex keys as strings 
--
...
function Graph:Dehash(value, hashDelim)
...
function Graph:Print()
   ...
   local delim = ""
   for i=1,col do delim = delim .. "-" end
   local hline = delim .. "|"
   ...
   for i = 1,self.num_v do printf("%s|", self:GetCellString(self.vertices[i], col)) 
      for j =1,self.num_v do printf("%s|", self:GetCellString(tab[i][j], col)) end
      printf("\n%s", hline)
   end
...
function Graph:DehashedTable()
   local tab = {}
   for i=1,self.num_v do tab[i] = {} end
